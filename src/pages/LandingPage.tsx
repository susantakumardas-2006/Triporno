    ...
    <div className="relative min-h-screen overflow-hidden bg-black flex flex-col">
      ...
      <nav className="relative z-20 px-6 py-6">
        <div className="rounded-full px-6 py-3 flex items-center justify-between max-w-5xl mx-auto liquid-glass">
          ...
          <button onClick={() => navigate('/auth/register')} className="rounded-full bg-white px-4 py-2 text-sm font-medium text-black">
            Register
          </button>
        </div>
      </nav>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 py-12 text-center -translate-y-[8%]">
        <div className="max-w-5xl w-full">
          <h1 className="text-white mb-8 tracking-tight text-4xl md:text-5xl lg:text-6xl leading-tight" style={{ fontFamily: '"Instrument Serif", serif' }}>
            SmartEd: Built for the curious
          </h1>

          <div className="liquid-glass rounded-3xl p-8 w-full max-w-md mx-auto">
            <div className="rounded-full bg-white/5 p-1 flex" role="tablist" aria-label="Choose your role">
              {navLinks.map((item, index) => (
                <button
                  key={item}
                  type="button"
                  className={`flex-1 text-center text-sm py-2.5 rounded-full transition-all duration-300 ${role === item ? 'bg-white text-black font-medium' : 'text-white/60 hover:text-white/80'}`}
            ...
            <form onSubmit={handleLogin} className="mt-4">
              ...
              <div className="space-y-3">
                <input
                  ...
                  placeholder="Email"
                  className="w-full rounded-full bg-white/10 px-4 py-3 text-white placeholder:text-white/40 outline-none"
                />
                <div className="flex items-center rounded-full bg-white/10 px-4 py-3">
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword((value) => !value)} className="text-white/70">
                    {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                </div>
              </div>
              <button type="submit" className="bg-white text-black rounded-full py-3 font-medium hover:bg-neutral-200 transition-colors w-full mt-4">
                Log in as {roleLabels[role]}
              </button>
              <div className="mt-3 text-center">
                <span className="text-white/60 text-xs">Need an account? </span>
                <button type="button" onClick={() => navigate('/auth/register')} className="text-white text-xs font-medium">
                  Register
                </button>
              </div>
            </form>
          ...
          <div className="max-w-xl w-full space-y-4 mt-8 mx-auto">
            <div className="liquid-glass rounded-full pl-6 pr-2 py-2 flex items-center gap-3">
              ...
              <button className="rounded-full bg-white p-2 text-black" type="button">
                <ArrowRight size={18} />
              </button>
            </div>
